...
import { sampleWithRequiredData } from '../device.test-samples';
...
describe('Device Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'd5008764-aa30-42e8-a82f-8025612527af' }], {
    ...
    expect(comp.devices()[0]).toEqual(expect.objectContaining({ id: 'd5008764-aa30-42e8-a82f-8025612527af' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'd5008764-aa30-42e8-a82f-8025612527af' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.devices()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'd5008764-aa30-42e8-a82f-8025612527af' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to deviceService', () => {
      ...
      vitest.spyOn(service, 'getDeviceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDeviceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.devicesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
