...
import { sampleWithRequiredData } from '../metric-sample.test-samples';
import { MetricSampleService } from '../service/metric-sample.service';

import { MetricSample } from './metric-sample';

vitest.useFakeTimers();

describe('MetricSample Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: MetricSample;
  let fixture: ComponentFixture<MetricSample>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(MetricSample);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'fd69406b-7fef-480a-a1db-0301c5494580' }], {
    ...
    expect(comp.metricSamples()[0]).toEqual(expect.objectContaining({ id: 'fd69406b-7fef-480a-a1db-0301c5494580' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'fd69406b-7fef-480a-a1db-0301c5494580' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.metricSamples()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'fd69406b-7fef-480a-a1db-0301c5494580' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to metricSampleService', () => {
      const entity = { id: 'fd69406b-7fef-480a-a1db-0301c5494580' };
      vitest.spyOn(service, 'getMetricSampleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMetricSampleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.metricSamplesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
