...
describe('AlertRule Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '3410c5dd-5c03-47e7-8879-77d7e4199ac7' }], {
    ...
    expect(comp.alertRules()[0]).toEqual(expect.objectContaining({ id: '3410c5dd-5c03-47e7-8879-77d7e4199ac7' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '3410c5dd-5c03-47e7-8879-77d7e4199ac7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.alertRules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '3410c5dd-5c03-47e7-8879-77d7e4199ac7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to alertRuleService', () => {
      ...
      vitest.spyOn(service, 'getAlertRuleIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.alertRulesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
