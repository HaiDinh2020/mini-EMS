...
import { sampleWithRequiredData } from '../alert-event.test-samples';
...
describe('AlertEvent Management Component', () => {
  ...
  let comp: AlertEvent;
  let fixture: ComponentFixture<AlertEvent>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AlertEvent);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '9804ef1a-eec2-47f1-b576-cdfa1e0d89d1' }], {
    ...
    expect(comp.alertEvents()[0]).toEqual(expect.objectContaining({ id: '9804ef1a-eec2-47f1-b576-cdfa1e0d89d1' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '9804ef1a-eec2-47f1-b576-cdfa1e0d89d1' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.alertEvents()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '9804ef1a-eec2-47f1-b576-cdfa1e0d89d1' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to alertEventService', () => {
      const entity = { id: '9804ef1a-eec2-47f1-b576-cdfa1e0d89d1' };
      vitest.spyOn(service, 'getAlertEventIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAlertEventIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.alertEventsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
