...
import { sampleWithRequiredData } from '../topology-link.test-samples';

import { TopologyLink } from './topology-link';

vitest.useFakeTimers();

describe('TopologyLink Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TopologyLink;
  let fixture: ComponentFixture<TopologyLink>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TopologyLink);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '118ecf33-e7af-464b-a0ee-70a59f67f520' }], {
    ...
    expect(comp.topologyLinks()[0]).toEqual(expect.objectContaining({ id: '118ecf33-e7af-464b-a0ee-70a59f67f520' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '118ecf33-e7af-464b-a0ee-70a59f67f520' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.topologyLinks()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '118ecf33-e7af-464b-a0ee-70a59f67f520' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to topologyLinkService', () => {
      const entity = { id: '118ecf33-e7af-464b-a0ee-70a59f67f520' };
      vitest.spyOn(service, 'getTopologyLinkIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTopologyLinkIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.topologyLinksParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
